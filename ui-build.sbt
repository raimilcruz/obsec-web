...
//directory that contains node application
val uiDirectory = "node"
...
//This line is commented because in Windows the npm process is not killed when one stops the play app
...
`ui-test` := {
  implicit val userInterfaceRoot = baseDirectory.value / uiDirectory
